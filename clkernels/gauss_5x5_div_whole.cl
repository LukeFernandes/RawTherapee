...
__kernel void gauss_5x5_div_whole( __global const float *oldsrc, __global const int *X, __global const int *Y,  const int W, const int H, const float c21, const float c20, const float c11, const float c10, const float c00, __global float *div, __global float *olddst) {
...
float temp_pre_division = ( c21 * ( oldsrc[clamp(index_equiv - (2 * W) - 1, 0, totalpix)]
